...
function getActionEntries(targetSet) {
  if (!targetSet) return [];
  const entries = Array.isArray(targetSet.actions)
    ? targetSet.actions
    : Array.isArray(targetSet.actionContents)
      ? targetSet.actionContents
      : [];
...
async function populateActionDropdowns(options = {}) {
  ...
  const renderActionOptions = (setName, presetActionName = "") => {
    ...
    const actions = getActionEntries(targetSet);
    if (actions.length === 0) {
      actionSelect.disabled = true;
      return;
    }
    ...
    if (presetActionName && actions.some(act => act.name === presetActionName)) {
